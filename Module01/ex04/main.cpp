...
/*   Updated: 2024/09/15 14:24:20 by kmatjuhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string	replace_str(std::string line, std::string erase, std::string replace_str) {
	int		index = 0;
	size_t	erase_length = erase.length();

	while ((size_t)index != std::string::npos) {
		index = line.find(erase);
		if (index == -1)
			break ;
		line.erase(index, erase_length);
		line.insert(index, replace_str);
	}
	return (line);
}
